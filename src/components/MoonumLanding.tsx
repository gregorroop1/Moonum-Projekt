      ...
      <div className="bg-brand-dark text-zinc-200 font-sans h-screen w-full m-0 flex items-center justify-center relative">
        ...
        <div className="absolute inset-0 p-8 md:p-16 grid grid-cols-2 grid-rows-[auto_1fr_auto] z-10 pointer-events-none">
          <div className="flex flex-col justify-center items-end text-right translate-y-16 col-start-2 row-start-2">
            <h1 className="font-['Michroma'] uppercase drop-shadow-[0px_12px_24px_rgba(0,0,0,1)] drop-shadow-[0px_0px_12px_rgba(0,0,0,0.8)]" style={{ fontSize: 'clamp(1.5rem, 4vw, 3.5rem)', lineHeight: '1.2' }}>
        ...
        <div className="w-screen h-screen flex items-center justify-center overflow-hidden [perspective:2000px]">
          <div className="flex items-center justify-center w-[800px] h-[500px] [transform-style:preserve-3d] max-lg:scale-[0.85] max-md:scale-[0.6] max-[500px]:scale-50 max-[380px]:scale-[0.42]">
            <div className="relative w-[800px] h-[500px] [transform-style:preserve-3d] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] will-change-transform" ref={wrapperRef}>
              ...
              <div className="absolute w-[200%] h-[200%] -top-1/2 -left-1/2 -translate-z-[50px] pointer-events-none [background-image:repeating-radial-gradient(circle_at_50%_50%,transparent_0,transparent_40px,rgba(255,255,255,0.05)_41px,transparent_42px)]"></div>
            </div>
          </div>
        </div>

        <div className="absolute bottom-8 left-1/2 w-[1px] h-[60px] bg-gradient-to-b from-zinc-200 to-transparent animate-flow"></div>
      </div>
    </>
  );
};
